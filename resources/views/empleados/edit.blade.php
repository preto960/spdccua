...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="mb-0">Editar Empleado ID: {{ $empleado->id }}</h1>
    ...
    <form action="{{ route('empleados.update', $empleado->id) }}" method="POST" class="bg-white p-4 rounded shadow-sm">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="cedula" class="form-label">Cédula</label>
            <input type="number" class="form-control @error('cedula') is-invalid @enderror" id="cedula" name="cedula" value="{{ old('cedula', $empleado->cedula) }}" required>
            @error('cedula')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('grado_instruccion') is-invalid @enderror" id="grado_instruccion" name="grado_instruccion" value="{{ old('grado_instruccion', $empleado->grado_instruccion) }}" required>
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('estatus') is-invalid @enderror" id="estatus" name="estatus" required>
                <option value="1" {{ old('estatus', $empleado->estatus) == '1' ? 'selected' : '' }}>Activo</option>
                <option value="0" {{ old('estatus', $empleado->estatus) == '0' ? 'selected' : '' }}>Inactivo</option>
            </select>
            @error('estatus')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
